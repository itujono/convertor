  ...
  try {
    ...
    const timeoutId = setTimeout(
      () => {
        ...
        if (retryCount < maxRetries) {
          toast.warning(
            `Upload timeout, retrying... (${retryCount + 1}/${maxRetries})`,
            {
    ...
    try {
      ...
      if (response.filePath) {
        ...
        throw new Error(
          "Invalid upload response - missing filePath and uploadId"
        );
      }
    } catch (error) {
      clearTimeout(timeoutId);
        ...
        if (actualFilePath && !actualFilePath.match(/^\d+-[a-z0-9]+$/)) {
          ...
          if (response.ok) {
            ...
            if (progressData.progress !== undefined) {
              ...
              const adjustedProgress = uploadId
                ? 50 + progressData.progress * 0.5
...
export function useUploadProgress() {
  ...
  useEffect(() => {
    const hasActiveUploads = uploadProgress.some(
      (p) => (!p.completed && !p.aborted) || p.converting
    );
    ...
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (hasActiveUploads) {
        e.preventDefault();
        e.returnValue =
          "You have uploads/conversions in progress. Are you sure you want to leave?";
  ...
  useEffect(() => {
    const allComplete =
      ...
      uploadProgress.every((p) => p.converted && !p.error && !p.aborted);
      ...
      uploadFile(
        ...
        (filePath, uploadId) => {
          setUploadProgress((prev) =>
            prev.map((item) =>
              item.fileId === file.id
                ? { ...item, completed: true, converting: true }
                : item
            )
          );
        ...
        (error) => {
          setUploadProgress((prev) =>
            prev.map((item) =>
              item.fileId === file.id ? { ...item, error } : item
            )
          );
        },
        isOnline,
  ...
  const abortUpload = async (fileId: string) => {
    ...
    setUploadProgress((prev) =>
      prev.map((item) =>
        item.fileId === fileId ? { ...item, aborted: true } : item
      )
    );
  ...
  const clearProgress = async () => {
    const activeUploadsCount = uploadProgress.filter(
      (p) => !p.completed && !p.aborted && !p.error
    ).length;
    ...
    try {
      ...
      if (
        abortResult.success &&
        abortResult.abortedCount &&
        abortResult.abortedCount > 0
      ) {
        console.log(
          `✅ Aborted ${abortResult.abortedCount} server-side uploads`
        );
      }
