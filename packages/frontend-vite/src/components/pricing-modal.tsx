...
import { useCreateCheckoutSession } from "@/lib/api-hooks";
...
export function PricingModal({ isOpen, onClose }: PricingModalProps) {
  ...
  const handleCheckout = async (plan: "monthly" | "yearly") => {
    ...
    try {
      const { checkoutUrl } = await createCheckoutSession.mutateAsync(plan);
      ...
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <p className="text-sm text-gray-600">
            Upgrade to Premium to make the most of our converter
          </p>
        </DialogHeader>
              ...
              <div>
                ...
                <div className="flex items-baseline gap-1">
                  <span className="text-xl font-bold">
                    ${premium.price?.monthly ?? 0}
                  </span>
                  <span className="text-gray-500 text-sm relative md:bottom-0.5">
                    /month
                  </span>
                </div>
              </div>
        ...
        <div className="mt-4 space-y-2">
          ...
          <ul className="space-y-3 text-gray-600">
            ...
            <li className="flex items-center">
              <Check className="h-5 w-5 text-green-500 mr-3 flex-shrink-0" />
              <span className="text-sm">
                Supports larger file sizes (up to {premium.maxFileSizeMB / 1024}
                GB)
              </span>
            </li>
            <li className="flex items-center">
